...
namespace AnarchyChess.Games
{
    ...
    {
        private readonly Dictionary<Type, string> _pieceTextureRegistry = new Dictionary<Type, string>();
        ...
        public GameManager RegisterPiece(Type pieceType, string pathTemplate)
        {
        ...
        public GameManager DeregisterPiece(Type pieceType)
        {
            var isPieceType = pieceType.GetInterfaces().Contains(typeof(IPiece));
            if (!isPieceType)
            {
        ...
        public GameManager SetRegistered(Type pieceType, string pathTemplate, bool register)
        {
            return register ? RegisterPiece(pieceType, pathTemplate) : DeregisterPiece(pieceType);
        }

        
        public Texture GetTexture(IPiece piece)
        {
